class CommunityPostService < BaseService
  ...
  def assign_admin_and_create_content_type
    @community.community_admins.create(account_id: @account.id, username: @community.slug.underscore, display_name: @community.name, email: @current_user.email, role: @current_user&.role&.name, is_boost_bot: true)
